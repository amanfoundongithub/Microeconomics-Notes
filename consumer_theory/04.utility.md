...
While the simplification seems kinda superficial, yet it is considered as a good approximation to the realistic consumer behaviors at least.

In most of economics discussion, we usually refer to the Cobb-Douglas utility function to describe a typical utility function.

## Marginal Utility - Definition

In real life, consumers don't usually go and think, "Is buying 4 enough? 6 enough?" but rather prefers to think  `marginally` i.e. consumers prefer to answer the following question:

_Given I have already purchased some units of a good, is it worth it for me to purchase one more unit?_

Such a question is not only asked by consumers, but producers as well when producting goods, which we will discuss in producer theory.

But this thinking is important, because this will lead us to a reason why economics is often called the dismal science - `opportunity cost`.

For now, let's define the marginal utility as follows:

**Definition:** Marginal utility $MU_i(x_1, x_2, ..., x_n)$ with respect to good #$i$ is defined as the rate at which the utility changes when the good #$i$ changes. Thus, we can mathematically define it as:

<p align = 'center'>
$MU_i(x_1, x_2, ..., x_n) = \frac{\Delta U(x_1, x_2, ..., x_n)}{\Delta x_i}$
</p>

If the curves are plotted in such a way that the utility becomes a continuous function, then we can approximate this to partial derivatives in the limit $\Delta x_i \rightarrow 0$:

<p align = 'center'>
$MU_i(x_1, x_2, ..., x_n) = \frac{\partial U(x_1, x_2, ..., x_n)}{\partial x_i}$
</p>

Marginal utility is an important concept because it allows consumers us to think marginally about how each unit of good we consume affects their utility.

- If consuming the next good does not increase utility ($MU_i(x_1, x_2, ..., x_n) \leq 0$ ), then consumers will tend to not consume that good, and that is what happens in practice: people consume until they can't get any more satisfaction from it!

- Marginaln utilites are sometimes called _decision boundaries_ in the economics, because they allow for decisions to be made and can set a boundary to the consumption of goods.

## Next?

In this unit, we discussed and introduced utility and marginal utility along with some examples of utility function. 

We will stop here for now, but the next topics are going to be interesting because there will be good amount of mathematics involved and topics will be more practical to the study as well.